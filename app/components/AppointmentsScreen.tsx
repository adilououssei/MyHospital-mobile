...
const AppointmentsScreen = ({ onNavigate, unreadCount = 0 }: AppointmentsScreenProps) => {
    ...
    const mapStatus = (backendStatus: string): 'pending' | 'confirmed' | 'past' | 'rejected' => {
        const statusMap: Record<string, 'pending' | 'confirmed' | 'past' | 'rejected'> = {
            'pending': 'pending', 'pending_payment': 'pending',
            'accepted': 'confirmed', 'confirmed': 'confirmed',
            'completed': 'past', 'cancelled': 'rejected', 'refused': 'rejected',
        };
    ...
    const handleRescheduleAppointment = (appointment: Appointment) => {
        onNavigate('doctorDetail', {
            doctor: {
                id: appointment.doctorId, name: appointment.doctorName,
                specialty: appointment.specialty, photo: appointment.doctorImage,
            ...
            { text: t('cancel'), style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        ...
                        Alert.alert(t('error'), error.message || t('aptErrLoad'));
                    }
                },
            }
        ]);
    ...
    const handleJoinVideoCall = (id: string) => {
        Alert.alert(t('aptJoinTitle'), t('aptJoinMsg'), [
            { text: t('cancel'), style: 'cancel' },
            { text: t('aptJoin'), onPress: () => console.log('Rejoindre vidéo:', id) }
        ]);
    };
    ...
    const renderAppointmentActions = (appointment: Appointment) => {
        if (appointment.status === 'pending') return (
            <View style={styles.appointmentActions}>
                <TouchableOpacity style={[styles.cancelButton, { backgroundColor: colors.inputBackground }]}
        ...
        if (appointment.status === 'confirmed' && appointment.consultationType === 'online') return (
            <View style={styles.appointmentActions}>
                <TouchableOpacity style={styles.joinVideoButton} onPress={() => handleJoinVideoCall(appointment.id)}>
            ...
            <View style={styles.tabsContainer}>
                {(['pending', 'confirmed', 'past', 'rejected'] as const).map(tab => (
                    <TouchableOpacity key={tab}
                        style={[styles.tab, { backgroundColor: colors.inputBackground }, activeTab === tab && styles.activeTab]}
                        onPress={() => setActiveTab(tab)}>
                        <Text style={[styles.tabText, { color: colors.subText }, activeTab === tab && styles.activeTabText]}>
                            {getStatusText(tab)}
                        </Text>
                    </TouchableOpacity>
                ))}
                                    ...
                                    {appointment.consultationType === 'hospital' && appointment.hospitalAddress && (
                                        <View style={styles.addressSection}>
                                            <View style={styles.addressRow}>
                                                ...
                                                <Text style={[styles.addressText, { color: colors.subText }]}>{appointment.hospitalAddress}</Text>
                                            </View>
                                            <TouchableOpacity style={styles.directionsButton}
                                                onPress={() => handleGetDirections(appointment.hospitalCoordinates!)}>
                                                <Ionicons name="navigate" size={18} color="#fff" />
                                                <Text style={styles.directionsButtonText}>{t('aptDirections')}</Text>
                                            </TouchableOpacity>
                                        </View>
                                    )}
                                </View>
