...
const NotificationsScreen = ({ onNavigate, onUpdateUnreadCount }: NotificationsScreenProps) => {
  ...
  const handleRefresh   = () => { setRefreshing(true); loadNotifications(true); };
  const handleLoadMore  = () => { if (!loadingMore && hasMore && !loading) { setLoadingMore(true); loadNotifications(false); } };
  ...
  const renderItem = ({ item }: { item: ApiNotification }) => {
    ...
    return (
      <TouchableOpacity
        style={[styles.card, { backgroundColor: colors.card }, !item.isRead && styles.cardUnread]}
        ...
        <View style={styles.cardBody}>
          <View style={styles.cardTop}>
            <Text style={[styles.cardTitle, { color: colors.text }, !item.isRead && styles.cardTitleUnread]} numberOfLines={1}>
              {item.title}
            </Text>
          ...
          <View style={styles.cardFooter}>
            ...
            <Text style={[styles.dateText, { color: colors.subText }]}>{dateText}</Text>
          </View>
        </View>
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.emptyState}>
        ...
        <TouchableOpacity style={styles.retryButton} onPress={() => { setLoading(true); loadNotifications(true); }}>
          <Text style={styles.retryButtonText}>{t('notifRetry')}</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
